...
# Ensure the script looks for the CSV in the repository's root folder with the correct name
file_path = os.path.join(os.getcwd(), "Diabetes Data.csv")

# Load dataset
@st.cache_data
def load_data():
    if not os.path.exists(file_path):
        st.error("❌ Error: 'Diabetes Data.csv' not found in the repository root folder. Please ensure the file is present and retry.")
        st.stop()
